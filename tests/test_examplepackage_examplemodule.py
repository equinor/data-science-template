...
def test_examplepackage_example_hello_world():
    assert examplemodule.hello_world() == "Hello World", "The Hello World strings should be the same"


def test_examplepackage_example_add_value_to_numpy():
...
def test_examplepackage_example_add_value_to_numpy_wrong_type():
    with pytest.raises(ValueError) as e_info:
        examplemodule.add_value_to_numpy([1, 1], 1)


def test_examplepackage_example_add_value_to_numpy_empty():
    with pytest.raises(ValueError) as e_info:
